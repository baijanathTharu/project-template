...
export const useEditor = (props: CustomEditorProps) => {
  ...
  const editor = useTiptapEditor({
    ...
    extensions: [
      ...CoreEditorExtensions({
        ...
        mentionConfig: {
          mentionSuggestions:
            mentionHandler.suggestions ??
            (() => Promise.resolve<IMentionSuggestion[]>([])),
  ...
  useEffect(() => {
    ...
    if (
      editor &&
      !editor.isDestroyed &&
      !editor.storage.imageComponent.uploadInProgress
    ) {
      try {
  ...
  useImperativeHandle(
    ...
    () => ({
      ...
      getDocument: () => {
        const documentBinary = provider?.document
          ? Y.encodeStateAsUpdate(provider?.document)
          : null;
      ...
      emitRealTimeUpdate: (message: TDocumentEventsServer) =>
