...
const createDragHandleElement = (): HTMLElement => {
  ...
  iconElement2.classList.value =
    'pointer-events-none text-custom-text-300 -ml-2.5';
...
export const nodeDOMAtCoords = (coords: { x: number; y: number }) => {
  ...
  for (const elem of elements) {
    ...
    if (
      (elem.matches('td > p:first-child') ||
        elem.matches('th > p:first-child')) &&
...
export const DragHandlePlugin = (
  ...
  const handleDragStart = (event: DragEvent, view: EditorView) => {
    ...
    if (node.matches('blockquote')) {
      ...
      nodePosForBlockQuotes = Math.max(
        0,
        Math.min(nodePosForBlockQuotes, docSize)
      );
  ...
  const view = (view: EditorView, sideMenu: HTMLDivElement | null) => {
    ...
    dragHandleElement.addEventListener('drag', (e) => {
      ...
      if (e.clientY < scrollThreshold.up) {
        ...
        const overflow =
          e.clientY - (window.innerHeight - scrollThreshold.down);
