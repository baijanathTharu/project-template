...
import { coreEditorAdditionalSlashCommandOptions } from '../../../ce/extensions';
// local types
  ...
  ({ query }: { query: string }): TSlashCommandSection[] => {
        ...
        items: [
          {
            ...
            command: ({ editor, range }) =>
              toggleTextColor(undefined, editor, range),
          },
          ...COLORS_LIST.map(
            (color) =>
              ({
                ...
                command: ({ editor, range }) =>
                  toggleTextColor(color.key, editor, range),
          ...
          ...COLORS_LIST.map(
            (color) =>
              ({
                ...
                command: ({ editor, range }) =>
                  toggleBackgroundColor(color.key, editor, range),
              } as ISlashCommandItem)
          ),
        ],
      },
    ...
    [
      ...
      const sectionToPushTo =
        SLASH_COMMAND_SECTIONS.find((s) => s.key === item.section) ??
        SLASH_COMMAND_SECTIONS[0];
