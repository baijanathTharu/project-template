  ...
  addKeyboardShortcuts() {
    return {
      Backspace: ({ editor }) => {
        ...
        try {
          const isParentNodeCallout: Predicate = (node) =>
            node.type === this.type;
          const parentNodeDetails = findParentNodeClosestToPos(
            $from,
            isParentNodeCallout
          );
          // Check if selection is empty and at the beginning of the callout
          if (empty && parentNodeDetails) {
            const isCursorAtCalloutBeginning =
              $from.pos === parentNodeDetails.start + 1;
            if (
              parentNodeDetails.node.content.size > 2 &&
              isCursorAtCalloutBeginning
            ) {
          ...
          console.error(
            'Error in performing backspace action on callout',
            error
          );
        }
