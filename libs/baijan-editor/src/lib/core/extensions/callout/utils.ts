...
// plane ui
import { TEmojiLogoProps } from '../../../../ui/emoji';
// types
import {
  EAttributeNames,
...
type TStoredLogoValue = Pick<
  TCalloutBlockAttributes,
  EAttributeNames.LOGO_IN_USE
> &
...
export const getStoredLogo = (): TStoredLogoValue => {
  const fallBackValues: TStoredLogoValue = {
    'data-logo-in-use': 'emoji',
    'data-emoji-unicode':
      DEFAULT_CALLOUT_BLOCK_ATTRIBUTES['data-emoji-unicode'],
    'data-emoji-url': DEFAULT_CALLOUT_BLOCK_ATTRIBUTES['data-emoji-url'],
  };

  if (typeof window !== 'undefined') {
    const storedData = sanitizeHTML(
      localStorage.getItem('editor-calloutComponent-logo')
    );
    if (storedData) {
      ...
      try {
        ...
        console.error(
          `Error parsing stored callout logo, stored value- ${storedData}`,
          error
        );
      ...
      if (parsedData.in_use === 'emoji' && parsedData.emoji?.value) {
        return {
          ...
          'data-emoji-url':
            ...
            DEFAULT_CALLOUT_BLOCK_ATTRIBUTES['data-emoji-url'],
        };
      }
      if (parsedData.in_use === 'icon' && parsedData.icon?.name) {
        return {
          ...
          'data-icon-name':
            ...
            DEFAULT_CALLOUT_BLOCK_ATTRIBUTES['data-icon-name'],
          'data-icon-color':
            parsedData.icon.color ||
            DEFAULT_CALLOUT_BLOCK_ATTRIBUTES['data-icon-color'],
        };
      }
    }
  }
...
export const getStoredBackgroundColor = (): string | null => {
  if (typeof window !== 'undefined') {
    return sanitizeHTML(
      localStorage.getItem('editor-calloutComponent-background')
    );
  }
  return null;
};
