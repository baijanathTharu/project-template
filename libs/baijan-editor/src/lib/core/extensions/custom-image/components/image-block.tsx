...
// extensions
import { CustoBaseImageNodeViewProps, ImageToolbarRoot } from '..';
// helpers
...
const ensurePixelString = <TDefault,>(
...
export const CustomImageBlock: React.FC<CustomImageBlockProps> = (props) => {
  ...
  const [hasTriedRestoringImageOnce, setHasTriedRestoringImageOnce] =
    useState(false);
  ...
  const handleImageLoad = useCallback(() => {
    ...
    if (editorContainer) {
      ...
      closestEditorContainer = img.closest(
        '.editor-container'
  ...
  const handleResize = useCallback(
    (e: MouseEvent | TouchEvent) => {
      if (!containerRef.current || !containerRect.current || !size.aspectRatio)
        return;
  ...
  const handleResizeEnd = useCallback(() => {
    ...
    updateAttributesSafely(
      size,
      'Failed to update attributes at the end of resizing:'
    );
  ...
  const showImageLoader =
    !(resolvedImageSrc || imageFromFileSystem) ||
  ...
  const showImageResizer =
    editor.isEditable && resolvedImageSrc && initialResizeComplete;
          ...
          <div
            className={cn(
              'absolute inset-0 border-2 border-custom-primary-100 pointer-events-none rounded-md transition-opacity duration-100 ease-in-out',
              {
                ...
                'opacity-0 group-hover/image-component:opacity-100':
                  !isResizing,
              }
            )}
          />
