...
const SideMenu = (options: SideMenuPluginProps) => {
  ...
  // side menu elements
  const { view: dragHandleView, domEvents: dragHandleDOMEvents } =
    DragHandlePlugin(options);
  const { view: aiHandleView, domEvents: aiHandleDOMEvents } =
    AIHandlePlugin(options);
  ...
  return new Plugin({
    ...
    view: (view) => {
      ...
      if (
        handlesConfig.dragDrop &&
        !editorSideMenu.querySelector('#drag-handle')
      ) {
    ...
    props: {
      handleDOMEvents: {
        mousemove: (view, event) => {
          ...
          if (
            node.parentElement?.matches('td') ||
            node.parentElement?.matches('th')
          ) {
            rect.left += 8;
          }
