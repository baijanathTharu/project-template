...
// helpers
import { findTableAncestor } from './common';
// types
...
export const insertTableCommand = (editor: Editor, range?: Range) => {
  ...
  if (range)
    ...
    editor.chain().focus().clearNodes().insertTable({ rows: 3, cols: 3 }).run();
};
...
export const toggleTextColor = (
  ...
  if (color) {
    if (range)
      editor.chain().focus().deleteRange(range).setTextColor(color).run();
