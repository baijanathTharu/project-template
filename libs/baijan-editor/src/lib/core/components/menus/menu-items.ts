...
type isActiveFunction<T extends TEditorCommands> = (
  params?: TCommandWithProps<T>
) => boolean;
type commandFunction<T extends TEditorCommands> = (
  params?: TCommandWithProps<T>
) => void;
...
export const StrikeThroughItem = (
  editor: Editor
): EditorMenuItem<'strikethrough'> => ({
  key: 'strikethrough',
  name: 'Strikethrough',
...
export const ImageItem = (editor: Editor): EditorMenuItem<'image'> => ({
  ...
  name: 'Image',
  isActive: () =>
    editor?.isActive('image') || editor?.isActive('imageComponent'),
  command: ({ savedSelection }) =>
    insertImage({
      editor,
      event: 'insert',
      pos: savedSelection?.from ?? editor.state.selection.from,
    }),
  icon: ImageIcon,
});
...
export const TextColorItem = (
  editor: Editor
): EditorMenuItem<'text-color'> => ({
  key: 'text-color',
  name: 'Color',
...
export const BackgroundColorItem = (
  editor: Editor
): EditorMenuItem<'background-color'> => ({
  key: 'background-color',
  name: 'Background color',
  isActive: ({ color }) =>
    editor.isActive('customColor', { backgroundColor: color }),
  command: ({ color }) => toggleBackgroundColor(color, editor),
  icon: Palette,
});

export const TextAlignItem = (
  editor: Editor
